...
ON ST_Within(N2.geom, I.coverage_area_3g)
WHERE N1.node_type LIKE '%%3G%%'
AND N1.node_type LIKE '%%VIVO%%'
...
'EMPTY TOWER 3G' AS centroid_type,
node_weight AS centroid_weight,
'' AS nodes,
0 AS cluster_weight,
0 AS cluster_size
FROM {schema}.{table_nodes}
WHERE node_type LIKE '%%3G%%'
AND node_type LIKE '%%VIVO%%') A
