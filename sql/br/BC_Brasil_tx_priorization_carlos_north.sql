...
tamano
FROM (
SELECT DISTINCT ON (C.centroid) C.centroid as ran_centroid, C.cluster_weight as ran_weight, C.cluster_size as ran_size,
...
AND a.tower_id::text not in (SELECT centroid from rural_planner_dev.clusters_north
union
SELECT centroid from rural_planner_dev.clusters_north_3g)
