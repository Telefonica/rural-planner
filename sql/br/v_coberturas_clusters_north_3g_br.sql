CREATE MATERIALIZED VIEW {schema}.v_coberturas_clusters_north_3g AS
SELECT
    d.centroid,
    n.centroid_name,
...
LEFT JOIN
    (
        SELECT
            (infrastructure_global_backup.tower_id)::text AS centroid_id,
            infrastructure_global_backup.internal_id      AS centroid_name
        FROM
            {schema}.infrastructure_global_backup
        UNION
        SELECT
