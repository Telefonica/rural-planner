...
CASE WHEN I.source IN ('FEMTOS', 'FIBER_PLANNED', 'IPT', 'MACROS', 'OIMR') THEN 0
        WHEN I.source IN ('REGIONAL', 'YOFC', 'ADVISIA', 'LAMBAYEQUE', 'REGIONAL') THEN 1
...
ON ST_Within(N1.geom::geometry, I.coverage_area_3g)
LEFT JOIN {schema}.{table_coverage} C
ON N2.node_id = C.settlement_id
WHERE (N1.node_type LIKE '%%TOWER 3G%%')
...
WHERE node_type LIKE '%%TOWER 3G%%') A
