    ...
    (
        ...
        ON
            ...
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_entel_2g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS entel_presence_2g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_entel_3g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS entel_presence_3g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_entel_4g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS entel_presence_4g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_bitel_2g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS bitel_presence_2g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_bitel_3g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS bitel_presence_3g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_bitel_4g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS bitel_presence_4g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
        ...
        FROM
            (
                SELECT
                    ...
                    CASE
                        WHEN (b.entel_2g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_entel_2g,
                    CASE
                        WHEN (b.entel_3g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_entel_3g,
                    CASE
                        WHEN (b.entel_4g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_entel_4g,
                    CASE
                        WHEN (b.bitel_2g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_bitel_2g,
                    CASE
                        WHEN (b.bitel_3g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_bitel_3g,
                    CASE
                        WHEN (b.bitel_4g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_bitel_4g,
                    CASE
                ...
                FROM
                    (
                        ...
                        ON
                            ...
                            c_1.nodes,
                            cv.settlement_id,
                            cv.bitel_2g_regulator,
                            cv.bitel_3g_regulator,
                            cv.bitel_4g_regulator,
                            cv.bitel_2g_indirect,
                            cv.bitel_3g_indirect,
                            cv.bitel_4g_indirect,
                            cv.bitel_2g_app,
                            cv.bitel_3g_app,
                            cv.bitel_4g_app,
                            cv.bitel_2g_corrected,
                            cv.bitel_3g_corrected,
                            cv.bitel_4g_corrected,
                            ...
                            cv.entel_2g_regulator,
                            cv.entel_3g_regulator,
                            cv.entel_4g_regulator,
                            cv.entel_2g_indirect,
                            cv.entel_3g_indirect,
                            cv.entel_4g_indirect,
                            cv.entel_2g_app,
                            cv.entel_3g_app,
                            cv.entel_4g_app,
                            cv.entel_2g_corrected,
                            cv.entel_3g_corrected,
                            cv.entel_4g_corrected,
