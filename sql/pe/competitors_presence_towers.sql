CREATE TABLE IF NOT EXISTS {schema}.competitors_presence_towers (
                        tower_id int,
                        bitel_2g_presence float,
                        bitel_3g_presence float,
                        bitel_4g_presence float,
                        claro_2g_presence float,
                        claro_3g_presence float,
                        claro_4g_presence float,
                        entel_2g_presence float,
                        entel_3g_presence float,
                        entel_4g_presence float,
...
ELSE SUM(population_bitel_2g)::numeric/SUM(population_corrected)::numeric END AS bitel_presence_2g,
CASE WHEN SUM(population_corrected) = 0 THEN 0
ELSE SUM(population_bitel_3g)::numeric/SUM(population_corrected)::numeric END AS bitel_presence_3g,
CASE WHEN SUM(population_corrected) = 0 THEN 0
ELSE SUM(population_bitel_4g)::numeric/SUM(population_corrected)::numeric END AS bitel_presence_4g,
...
CASE WHEN SUM(population_corrected) = 0 THEN 0
ELSE SUM(population_entel_2g)::numeric/SUM(population_corrected)::numeric END AS entel_presence_2g,
CASE WHEN SUM(population_corrected) = 0 THEN 0
ELSE SUM(population_entel_3g)::numeric/SUM(population_corrected)::numeric END AS entel_presence_3g,
CASE WHEN SUM(population_corrected) = 0 THEN 0
ELSE SUM(population_entel_4g)::numeric/SUM(population_corrected)::numeric END AS entel_presence_4g,
...
FROM (
        ...
        CASE WHEN bitel_2g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_bitel_2g,
        CASE WHEN bitel_3g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_bitel_3g,
        CASE WHEN bitel_4g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_bitel_4g,
        ...
        CASE WHEN entel_2g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_entel_2g,
        CASE WHEN entel_3g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_entel_3g,
        CASE WHEN entel_4g_corrected IS TRUE THEN population_corrected ELSE 0 END AS population_entel_4g,
