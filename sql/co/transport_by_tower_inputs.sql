...
WHERE S2.source IN ({owners}) AND S1.source NOT IN ({sources_omit}) --AND  (S1.type<>'FIJA' OR S1.type IS NULL) 
--AND  (S2.type<>'FIJA' OR S2.type IS NULL)
ORDER BY S1.tower_id, distance
) A
WHERE fiber_1 IS FALSE
AND radio_1 IS FALSE
AND (radio_2 IS TRUE OR fiber_2 IS TRUE)
ORDER BY tower_id, fiber_2 DESC, distance
