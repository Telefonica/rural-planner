...
case when cp.centro_poblado is not null then cp.centro_poblado else i.internal_id END AS nombre_centroide,
    ...
    SUM(k.escuelas) as escuelas,
    p.precipitations as lluvias,
    bool_or(k.b2bcoffee) as b2bcoffee,
...
LEFT JOIN
    {schema}.v_centros_poblados_kpis k
ON
    ((
...
LEFT JOIN {schema}.clusters_precipitations p
on p.centroid=b.centroid)
GROUP BY
    c.centroid,
    b.cluster_size,
    b.cluster_weight,
