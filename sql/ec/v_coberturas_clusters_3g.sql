...
CREATE OR REPLACE VIEW {schema}.v_coberturas_clusters_3g as (
SELECT 
D.centroid,
N.centroid_name,
...
FROM (
     ...
     CASE WHEN movistar_3g_corrected IS TRUE OR movistar_4g_corrected IS TRUE THEN 0
        ...
        FROM (
                ...
                FROM {schema}.clusters_3g
                
                UNION (SELECT centroid, cluster_weight, centroid AS nodes from {schema}.clusters_3g) 
                ) C
