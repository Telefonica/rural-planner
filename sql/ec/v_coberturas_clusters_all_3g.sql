CREATE OR REPLACE VIEW {schema}.v_coberturas_clusters_all_3g ( 
centroid,
centroid_name,
                ...
                FROM
                    (
                        ...
                        FROM
                            ((((
                                SELECT
                                    clusters_3g.centroid,
                                    clusters_3g.cluster_weight,
                                    clusters_3g.cluster_size,
                                    CASE
                                        WHEN (clusters_3g.nodes = ''::text)
                                        THEN NULL::text
                                        ELSE btrim(unnest(string_to_array(REPLACE(clusters_3g.nodes,
                                            ''''::text, ''::text), ','::text)))
                                    END AS nodes
                                FROM
                                    {schema}.clusters_3g
                                ...
                                SELECT
                                    ...
                                    clusters_3g.cluster_weight,
                                    clusters_3g.cluster_size,
                                    clusters_3g.centroid AS nodes
                                FROM
                                    {schema}.clusters_3g) c_1
                        LEFT JOIN
                            {schema}.coverage cv
                        ON
                            ((
