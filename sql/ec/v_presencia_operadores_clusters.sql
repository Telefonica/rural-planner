    ...
    (
        ...
        ON
            ...
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_cnt_2g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS cnt_presence_2g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_cnt_3g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS cnt_presence_3g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
                ELSE ROUND(((SUM(c.population_cnt_4g))::NUMERIC / (SUM(c.population_corrected))::
                    NUMERIC), 2)
            END AS cnt_presence_4g,
            CASE
                WHEN (SUM(c.population_corrected) = 0)
                THEN (0)::NUMERIC
        ...
        FROM
            (
                SELECT
                    ...
                    CASE
                        WHEN (b.cnt_2g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_cnt_2g,
                    CASE
                        WHEN (b.cnt_3g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_cnt_3g,
                    CASE
                        WHEN (b.cnt_4g_corrected IS TRUE)
                        THEN (b.population_corrected)::DOUBLE PRECISION
                        ELSE (0)::DOUBLE PRECISION
                    END AS population_cnt_4g,
                    CASE
