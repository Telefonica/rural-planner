...
case when cp.centro_poblado is not null then cp.centro_poblado else i.internal_id END AS nombre_centroide,
    ...
    string_agg(DISTINCT k.parroquia, ' , '::text) AS parroquias,
    string_agg(DISTINCT k.canton, ' , '::text)       AS cantones,
...
FROM
    ...
    (
        SELECT
            clusters_3g_links.centroid,
            clusters_3g_links.geom_centroid,
            clusters_3g_links.geom_node,
            clusters_3g_links.geom_line,
            CASE
                WHEN (((clusters_3g_links.node_2_id IS NULL)
                        OR  (LENGTH(btrim(clusters_3g_links.node_2_id)) = 0))
                    OR  (clusters_3g_links.node_2_id = ''::text))
                THEN clusters_3g_links.centroid
                ELSE clusters_3g_links.node_2_id
            END AS node
        FROM
            {schema}.clusters_3g_links) c
LEFT JOIN
...
LEFT JOIN {schema}.node_table_3g a
ON
    ((
