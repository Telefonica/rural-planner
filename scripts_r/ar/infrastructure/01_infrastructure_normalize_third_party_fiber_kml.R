...
source('~/shared/rural_planner/sql/ar/infrastructure/exportFiber.R')

#LOAD INPUTS
#Unzip kmz file to open get .kml file
kml_files <- list.files(input_path_infrastructure,full.names = F,recursive = F,pattern='\\.kml$',ignore.case = T)

file.rename(paste(input_path_infrastructure,kml_files, sep="/"), paste(input_path_infrastructure,gsub('-','_', kml_files), sep="/"))
...
for (i in (1:length(kml_files))){
  ...
  dflines_int$source <- paste0(toupper(gsub(".kml","",kml_file)),"_LINES")
  
  dflines_int$source_file <- kml_file
  ...
  assign(paste("dflines",tolower(gsub(".kml","",kml_file)),sep="_"), dflines_int)
  
  }
  
 
  print(paste(kml_file," trace processed.", sep=""))
  ...
  #dfpoints_int$location_detail <- gsub(" - NA","",dfpoints_int$location_detail)
  
  dfpoints_int$internal_id <- dfpoints_int$name
  
  dfpoints_int$source <- paste0(toupper(gsub(".kml","",kml_file)),"_POINTS")
  
  dfpoints_int$source_file <- kml_file
  ...
  assign(paste("dfpoints",tolower(gsub(".kml","",kml_file)),sep="_"), dfpoints_int)
  }
  
  print(paste(Sys.time(),"/",kml_file," pops processed.", sep=""))
...
third_party_pops$tx_3g <- FALSE
third_party_pops$tx_third_pty <- FALSE
...
exportFiber(schema_dev, table_third_party_lines, table_third_party_points, third_party_traces, third_party_pops)
