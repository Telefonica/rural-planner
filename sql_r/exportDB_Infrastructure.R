exportDB_Infrastructure <- function(schema, table, df){
  ...
  dbWriteTable(con, 
               c(schema,table), 
               value = data.frame(df), row.names = F, append= T)
  ...
  query <- paste("ALTER TABLE ", schema,".",table, " ADD COLUMN geom GEOMETRY ", sep = "")
  dbGetQuery(con,query)
