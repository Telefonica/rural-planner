exportNormalization <- function(schema_dev, output_table_name, coverage){
    ...
    query<-paste0('DROP TABLE IF EXISTS ',schema_dev,'.',output_table_name)
    dbGetQuery(con,query)
    
    dbWriteTable(con, 
                 c(schema_dev,output_table_name), 
    ...
    for (i in 2:length(names_coverage)){
      query <- paste0("ALTER TABLE ",schema_dev,".",output_table_name,
