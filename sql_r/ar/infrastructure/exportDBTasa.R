exportDBTasa <- function(schema, table, tasa){
    ...
    dbWriteTable(con, 
                 c(schema,table), 
                 value = data.frame(tasa), row.names = T, append= F)
    ...
    query <- paste(" CREATE INDEX tower_tasa_gix ON ", schema, ".", table, "  USING GIST(geom)", sep = "")
    dbGetQuery(con,query)
