exportDBtasaFixed <- function(schema, table, tasa_fixed){
    ...
    dbWriteTable(con, 
                 c(schema,table), 
                 value = data.frame(tasa_fixed), row.names = T, append= F)
    ...
    query <- paste("ALTER TABLE ", schema,".",table, " ALTER COLUMN coverage_area_2g TYPE GEOMETRY USING CAST(coverage_area_2g AS GEOMETRY)", sep = "")
    dbGetQuery(con,query)
    ...
    query <- paste("UPDATE ", schema,".",table, " SET geom = ST_SetSRID(ST_MakePoint(longitude, latitude), 4326)::GEOGRAPHY ", sep = "")
    dbGetQuery(con,query)
    
    query <- paste("UPDATE ", schema, ".", table, " SET subtype = replace(subtype,'á','a')", sep = "")
    dbGetQuery(con,query)
