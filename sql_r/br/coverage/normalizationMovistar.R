 # MOVISTAR
 normalizationMovistar <- function(schema, table_settlements, municipality, table_vivo){
 
        drv <- dbDriver("PostgreSQL")
        con <- dbConnect(drv, dbname = dbname,
                 host = host, port = port,
                 user = user, password = pwd) 
        query <- paste0("ALTER TABLE ", schema_dev,".", table_settlements," ALTER COLUMN geom TYPE geometry USING geom::geometry")
        ...
        query <- paste0("SELECT rt.settlement_id,
                          ...
                          FROM (SELECT * FROM ",schema,".",table_settlements,"
                                WHERE admin_division_2_id='", municipality,"') rt
                          LEFT JOIN LATERAL
                                  (SELECT *
